# Documentation

This index separates current guidance from historical work records. When code
and documentation disagree, use the following precedence:

1. accepted architecture decisions and current specifications;
2. current code and regression tests;
3. active or deferred requests;
4. archived requests and bootstrap records.

Archived material preserves rationale and provenance but is not an instruction
to reproduce an older implementation.

## Current guidance

- [Contributing](../CONTRIBUTING.md)
- [Legacy template imports](legacy-imports.md)
- [Outstanding clinical review](clinical-review.md)
- [Architecture decisions](adr/README.md)

## Specifications

- [Adolescent Hygiene interactive mapping](specs/adolescent-hygiene-interactive-template-mapping.md)
- [Adult Hygiene 2021 interactive mapping](specs/adult-hygiene-2021-interactive-template-mapping.md)
- [Adult Hygiene 2026 unified encounter](specs/adult-hygiene-2026-unified-encounter.md)
- [Oral hygiene methods used at home](specs/oral-hygiene-methods.md)
- [Rapid Entry v1 and chairside usability checklist](specs/rapid-entry-v1.md)
- [Periodontal classification decision table](specs/periodontal-classification-decision-table.md)
- [Recare Exam interactive mapping](specs/recare-exam-interactive-template-mapping.md)

## Reviews and requests

- [Adult Hygiene terminology review](reviews/adult-hygiene-terminology-review.md)
- [Request index](requests/README.md)
- [Archived request index](requests/archive/README.md)

## Historical material

- [Documentation archive](archive/README.md)
- [Legacy imported JSX](../legacy/imported-jsx/README.md)
- [Archived import summary previews](reference/archive/import-summary-previews.md)
